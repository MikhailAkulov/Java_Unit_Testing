...
public class CalculatorTestDep {
    ...
    public static void main(String[] args) {
        ...
        if (8 != Calculator2.calculation(2, 6, '+')) {
            throw new AssertionError("Ошибка в методе");
        }

        if (0 != Calculator2.calculation(2, 2, '-')) {
            throw new AssertionError("Ошибка в методе");
        }

        if (14 != Calculator2.calculation(2, 7, '*')) {
            throw new AssertionError("Ошибка в методе");
        }

        if (2 != Calculator2.calculation(100, 50, '/')) {
            throw new AssertionError("Ошибка в методе");
        }
        ...
        assert 8 == Calculator2.calculation(2, 6, '+');
        assert 0 == Calculator2.calculation(2, 2, '-');
        assert 14 == Calculator2.calculation(2, 7, '*');
        assert 2 == Calculator2.calculation(100, 50, '/');

        // Проверка базового функционала с целыми числами, с использованием утверждений AssertJ:
        assertThat(Calculator2.calculation(2, 6, '+')).isEqualTo(8);
        assertThat(Calculator2.calculation(2, 2, '-')).isEqualTo(0);
        assertThat(Calculator2.calculation(2, 7, '*')).isEqualTo(14);
        assertThat(Calculator2.calculation(100, 50, '/')).isEqualTo(2);

        // Проверка ожидаемого исключения, с использованием утверждений AssertJ:
        assertThatThrownBy(() ->
                Calculator2.calculation(8, 4, '_')
        ...
        assertThat(Calculator2.pow(10, 0)).isEqualTo(1);
        assertThat(Calculator2.pow(10, 1)).isEqualTo(10);
        assertThat(Calculator2.pow(2, 8)).isEqualTo(256);
        ...
        assertThatThrownBy(() ->
                Calculator2.calculatingDiscount(-1000, 50))
                .isInstanceOf(ArithmeticException.class)
        ...
        assertThatThrownBy(() ->
                Calculator2.calculatingDiscount(1000, -10))
                .isInstanceOf(ArithmeticException.class)
        ...
        // Ожидаемый результат: ошибка ArithmeticException с описанием "Скидка должна быть в диапазоне от 0 до 100%"
        assertThatThrownBy(() ->
                Calculator2.calculatingDiscount(1000, 101))
                .isInstanceOf(ArithmeticException.class)
                .hasMessage("Скидка должна быть в диапазоне от 0 до 100%");
        ...
        assertThat(Calculator2.calculatingDiscount(1000, 25)).isEqualTo(750);
        assertThat(Calculator2.calculatingDiscount(1000, 100)).isEqualTo(0);
        assertThat(Calculator2.calculatingDiscount(1000, 0)).isEqualTo(1000);


        // Проверка максимального и минимального значения, которое может хранить тип int
        assert Integer.MAX_VALUE == Calculator2.calculation(Integer.MAX_VALUE - 1, 1, '+');
        assert Integer.MIN_VALUE == Calculator2.calculation(Integer.MIN_VALUE + 1, -1, '+');

        // Проверка переполнения
        try {
            Calculator2.calculation(Integer.MAX_VALUE, 1, '+');
        ...
        try {
            Calculator2.calculation(Integer.MIN_VALUE, -1, '+');
            assert false; // Если мы здесь, значит, ожидаемого исключения не было
        } catch (ArithmeticException e) {
            assert true; // Мы ожидаем исключение ArithmeticException при переполнении
        }
